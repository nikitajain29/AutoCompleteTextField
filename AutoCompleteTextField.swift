...
public class AutoCompleteTextField:UITextField {
    ...
    private func setupAutocompleteTable(view:UIView){
        ...
        //let tableView = UITableView(frame: CGRect(x: self.frame.origin.x, y: self.frame.origin.y, width: self.frame.height, height: screenSize.width - (self.frame.origin.x * 2), 30.0))
...
//        let tableView = UITableView(frame: CGRectMake(5,38, screenSize.width - (self.frame.origin.x * 2), 30.0))
//        tableView.dataSource = self
//        tableView.delegate = self
//        tableView.rowHeight = autoCompleteCellHeight
...
extension AutoCompleteTextField: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        PlaceId = autoPlaceId![indexPath.row]
        print(PlaceId)
        if let selectedText = cell?.textLabel?.text {
            self.text = selectedText
            GooglePlaceSelectedText = selectedText
            self.resignFirstResponder()
